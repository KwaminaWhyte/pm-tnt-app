...
export default function PackageDetailsScreen() {
  ...
  return (
    <ThemedView className="flex-1 bg-slate-50 dark:bg-slate-950" style={{ paddingTop: top }}>
      <Stack.Screen
        options={{
          headerShown: false,
        }}
      ...
      <ScrollView className="flex-1">
        ...
        <View className="p-4">
          ...
          <View className="flex-row justify-between items-center mb-6">
            <View className="flex-row items-center">
              ...
              <ThemedText className="ml-2">
                {package_data?.duration?.days} Days, {package_data?.duration?.nights} Nights
              </ThemedText>
            </View>
          ...
          {package_data?.budget?.breakdown && (
            <View className="mb-6">
              <ThemedText className="text-lg font-semibold mb-3">Budget Breakdown</ThemedText>
              <View className="bg-white dark:bg-slate-900 p-4 rounded-lg">
                {Object.entries(package_data.budget.breakdown).map(([key, value]: [string, any]) => (
          ...
          {/* Transportation */}
          {package_data?.transportation?.details && package_data.transportation.details.length > 0 && (
            <View className="mb-6">
              <ThemedText className="text-lg font-semibold mb-3">Transportation</ThemedText>
              {package_data.transportation.details.map((transport: any, index: number) => (
                <View key={index} className="bg-white dark:bg-slate-900 p-4 rounded-lg mb-2">
                  <View className="flex-row items-center mb-2">
                    <MaterialCommunityIcons 
                      ...
                      color="#eab308" 
                    />
                    <ThemedText className="ml-2 font-semibold">{transport.type}</ThemedText>
                  </View>
          ...
          {/* Itinerary */}
          <View className="mb-6">
            <ThemedText className="text-lg font-semibold mb-3">Itinerary</ThemedText>
            {package_data?.itinerary?.map((day: any) => (
              <View key={day._id} className="mb-4 bg-white dark:bg-slate-900 p-4 rounded-lg">
          ...
          {/* What's Included */}
          <View className="mb-6">
            <ThemedText className="text-lg font-semibold mb-3">What's Included</ThemedText>
          ...
          {/* What's Not Included */}
          <View className="mb-6">
            <ThemedText className="text-lg font-semibold mb-3">What's Not Included</ThemedText>
          ...
          <View className="mb-6">
            <ThemedText className="text-lg font-semibold mb-3">Terms and Conditions</ThemedText>
          ...
          {package_data?.startDates && package_data.startDates.length > 0 && (
            <View className="mb-6">
              <ThemedText className="text-lg font-semibold mb-3">Available Start Dates</ThemedText>
          ...
          {/* Seasonal Pricing */}
          {package_data?.seasonalPricing && package_data.seasonalPricing.length > 0 && (
            <View className="mb-6">
              <ThemedText className="text-lg font-semibold mb-3">Seasonal Pricing</ThemedText>
              {package_data.seasonalPricing.map((season: any, index: number) => (
                <View 
                  key={index}
                ...
                >
                  <View className="flex-row justify-between items-center">
                    <ThemedText>
                      {moment(season.startDate).format("MMM D")} - {moment(season.endDate).format("MMM D, YYYY")}
                    </ThemedText>
                    <ThemedText className="text-yellow-500 font-semibold">
          ...
          {/* Participants */}
          <View className="mb-6">
            <ThemedText className="text-lg font-semibold mb-3">Participants</ThemedText>
            <View className="bg-white dark:bg-slate-900 p-4 rounded-lg">
              <View className="flex-row justify-between items-center mb-2">
                <ThemedText>Minimum</ThemedText>
                <ThemedText className="font-semibold">{package_data?.minParticipants}</ThemedText>
              </View>
              <View className="flex-row justify-between items-center">
                <ThemedText>Maximum</ThemedText>
                <ThemedText className="font-semibold">{package_data?.maxParticipants}</ThemedText>
              </View>
            </View>
          </View>
        </View>
